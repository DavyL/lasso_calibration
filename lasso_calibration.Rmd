...
CompareDimCriterionVarNP <- function(
  ...
  result_dim_var_np = data.frame(
    p = c(rep(prange[1], max(prange[1]-1, 0))),
    dim_AIC = c(rep(NA, prange[1]-1)),   ##Average dimesnsion of AIC
...
CompareDimCriterionVarSigma <- function(
  ...
  for(s in 1:length(sigmarange)){
    results_dim_sigma <- GetDimComparison(n=100, p = p, sigma = sigmarange[s], B = 20, N= 20, p0 = p0)
...
for(k in krange){
  new_row_dim_sigma = data.frame(
    k = k,
    comp_criterion = CompareDimCriterionVarSigma(n = 100, p = 50, p0 = k, sigmarange = sigmarange) 
  ) 
...
rgl.snapshot("comp_all_crits_dim_var_sigma.png")
```
