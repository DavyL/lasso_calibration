...
```{r}
likelihood <- function(betal, Yl, sigma2 = 1.0){ ##Not used
  S<-0.0
...
plot(result_var_np$err_AIC, x = prange, main = "Avg l2 error of AIC", ylab = "Avg error", xlab = "p")
plot(result_var_np$err_BIC, x = prange, main = "Avg l2 error of BIC", ylab = "Avg error", xlab = "p")
plot(result_var_np$err_CV, x = prange, main = "Avg l2 error of CV", ylab = "Avg error", xlab = "p")
...
```
![Comparison of which criterion gives the lambda closest to best lambda for varying dimensionality and sparsity](comp_all_crits.png)

From this graph ( $n = 50, p \in [5, 100], p_0 \in [5, 50]$ ) we have that AIC outperforms clearly both CV and BIC in the large $p$, large $p_0$ area (high-dimension, low sparsity). In the low-dimensionality, high sparsity area (my guess : $p_0 < p/2$) all criterions have a similar performance. 
...
```
![Comparison of the dimension identified depending on dimensionality and sparsity](comp_all_crits_dim.png)
We can observe that when $p=p0$, then all criterions perform equally. This probably comes from (TODO : check and add details) the fact when the sparsity of the input vector is constant and $p=p0$, hence AIC and BIC can be proved equivalent. Also, in the Gaussian case this certainly amounts to solving a least square to find the CV solution which is the same as for AIC or BIC.
...
```
 ...
 ![Comparison of the dimension identified depending on dimensionality and sparsity for sparse low dimension](comp_all_crits_low_dim_sparse.png)

#Influence of noise

We are now left with having to measure the influence of the noise. In order to do that we will plot graphs of dimensionality identified (or distance w.r.t true $\lambda$) where the parameters are $(\sigma, n/p)$ or $(\sigma, k)$.
#Dimensionality
We compute dimension of the estimated model w.r.t to criterions for varying $sigma$ and $k$
```{r}
...
```
![Comparison of dimension estimated for criterions with varying noise and sparsity in a high-dimension setting](comp_all_crits_dim_var_sigma_low_dim.png)
We can make several observations from the plot in a low-dimension setting (for n = 50, p = 100, sigmarange = [0,5] ) :
1) All criterions overfit (include too many parameters) the data. CV is the best among three as it consistently identifies a dimension between 2 and 3 times the true dimension.
2) There is a cliff given by some $\sigma_i = \phi_i(p_0)$  AIC and BIC perform nearly no dimension reduction (BIC makes dimension reduction soon) which is for small noise (!)
3) In a situation without noise CV is the best, when noise is quite large BIC performs equivalently to CV (but is faster)
...
```
![Comparison of dimension estimated for criterions with varying noise and sparsity in a low dimension setting](comp_all_crits_dim_var_sigma_low_dim.png)
We can make several observations from the plot (for n = 50, p = 100, sigmarange = [0,5] ) :
...
```
![Comparison of criterion with varying noise and dimensionality](comp_all_crits_var_sigma_p.png)
